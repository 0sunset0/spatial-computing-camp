...
Day 2~6에서 발전시켜온 `SpatialCampApp/` 프로젝트(xcodegen 기반)에 캡스톤 기능을 이어서 추가합니다. 새 프로젝트를 만들지 마세요. 기존 구조는 `ARViewContainer.swift`(SwiftUI ↔ UIKit 어댑터, 세션/설정 담당)와 `ARCoordinator.swift`(델리게이트/제스처 이벤트 반응 담당)로 나뉘어 있고, 각 파일 안에서도 역할별로 작은 private 함수로 더 쪼개져 있습니다 — `ARViewContainer.swift`는 `makeSessionConfiguration()`/`configureDebugOptions(for:)`/`setupTapGesture(on:coordinator:)`(+ Day 6에서 추가한 `subscribeToCollisionEvents(on:coordinator:)`), `ARCoordinator.swift`는 `raycastResult(for:in:)`/`makeSphereEntity()`/`place(_:at:in:)`. 새 코드를 추가할 때는 "설정이냐 이벤트 반응이냐"로 먼저 파일을 고르고, 그 안에서도 가장 역할이 비슷한 함수를 찾아 넣거나(예: 엔티티 속성 추가는 `makeSphereEntity()`, 배치 시점 로직은 `place(_:at:in:)`) 새로운 역할이면 같은 패턴으로 새 private 함수를 만드세요. 필요하면 이 둘 외에 새 Swift 파일(예: 새 View, 새 System)을 추가해도 됩니다. 새 파일을 추가했다면 `cd SpatialCampApp && xcodegen generate`로 프로젝트를 갱신하세요. 코드를 바꿀 때마다 `xcodebuild -project SpatialCampApp.xcodeproj -scheme SpatialCampApp -destination 'generic/platform=iOS Simulator' build`로 컴파일을 검증하고, `BUILD SUCCEEDED`가 나온 뒤에 다음 단계로 넘어가세요. 실제 동작 확인은 실기에서 Xcode로 빌드·실행해야 한다는 점을 계속 상기시켜 주세요.
...
- 감정/상태에 따라 다른 색의 파티클 구슬을 띄우는 미니 감정 표현 시스템 (감정 하나만, 파티클 하나만으로 축소)
...
**가독성**: 체크리스트나 설계 옵션을 비교해서 보여줄 때는 긴 문단보다 표를 우선하세요.
