...
2. **개념 설명** — 그날의 핵심 개념을 대화창에서 먼저 설명합니다.
3. **코드 설명** — 실제로 작성할 코드를 대화창에 보여주며 각 부분이 무엇을 하는지 짚어줍니다.
4. **프로젝트에 실제로 코딩** — Day 2부터는 코드를 대화창에만 보여주지 않고, 작업 폴더의 실제 iOS 앱 프로젝트(`SpatialCampApp/`)에 직접 작성/수정하고 `xcodebuild`로 컴파일까지 검증합니다.
5. **체크포인트 퀴즈** — 객관식 4지선다 3문제를 `AskUserQuestion` 도구로 한 문제씩 진행합니다 (방향키/클릭으로 선택, 채점이 아니라 피드백 중심).
6. **다음 Day로 자동 진행** — 퀴즈가 끝나면 "다음" 또는 "완료"라고 입력하기만 하면, 슬래시 명령을 다시 칠 필요 없이 다음 Day 스킬이 자동으로 이어집니다.
