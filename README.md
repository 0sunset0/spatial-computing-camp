...
2. **개념 → 코드 → 퀴즈를 개념별로 반복** — 핵심 개념을 하나씩 순서대로 설명하고, 그 개념의 코드를 설명한 뒤, 그 개념에 대응하는 퀴즈 1문항을 `AskUserQuestion`으로 바로 진행합니다 (방향키/클릭으로 선택, 채점이 아니라 피드백 중심). 모든 개념을 다 설명한 뒤 퀴즈를 몰아서 내지 않고, 완전 초보자도 따라올 수 있게 비유를 곁들여 설명합니다.
3. **프로젝트에 실제로 코딩** — Day 2부터는 모든 개념+퀴즈가 끝난 뒤, 코드를 대화창에만 보여주지 않고 작업 폴더의 실제 iOS 앱 프로젝트(`SpatialCampApp/`)에 직접 작성/수정하고 `xcodebuild`로 컴파일까지 검증합니다.
4. **다음 Day로 자동 진행** — 모든 게 끝나면 "다음" 또는 "완료"라고 입력하기만 하면, 슬래시 명령을 다시 칠 필요 없이 다음 Day 스킬이 자동으로 이어집니다.
